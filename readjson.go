...
func readSMARTctl(logger log.Logger, device string) (gjson.Result, bool) {
	...
	out, err := exec.Command(*smartctlPath, "--json", "--info", "--health", "--attributes", "--tolerance=verypermissive", "--nocheck=standby", "--format=brief", "--log=error", "--quietmode=errorsonly", device).Output()
	if err != nil {
